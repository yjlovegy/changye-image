...
import { normalizeInpaintTuning, validateInpaintTuning, modeStrength, type InpaintTuning, type InpaintMode } from '@/backends/inpaintTuning';
...
const tuning=ref(normalizeInpaintTuning(localProfiles[workflow.value])),stepInput=ref(''),cfgInput=ref(''),nativeSize=ref(false);
...
function selectMode(value:string){tuning.value.mode=value as InpaintMode;tuning.value.denoise=modeStrength[value as InpaintMode];}
...
async function generate(){
 ...
 try{const mask=await maskBlob();if(current.signal.aborted)throw imageAbortError();
  const next=await props.run({workflow:workflow.value,mask,instruction:instruction.value.trim(),seed:seed.value,context:options.context,tuning:options},current.signal);
 ...
 <ModalMask :open="true" @close="close"><section ref="panel" class="inpaint-editor" role="dialog" aria-modal="true" aria-label="手动局部重绘" @keydown="keydown">
  ...
  <div v-if="!showResult" class="editor-layout">
   ...
   <aside class="settings-column"><div class="source-chip"><Icon name="generate"/><div><strong>已载入当前图片</strong><span>{{ready?`${width} × ${height}`:'读取中…'}} · 原图保留</span></div></div>
    ...
    <div class="field"><span class="field-label">重绘模式</span><BbiSelect :model-value="tuning.mode" @update:model-value="selectMode" :options="[{value:'touchup',label:'轻微修补'},{value:'repair',label:'结构修复'},{value:'replace',label:'内容替换'}]" aria-label="重绘模式" :disabled="disabled"/></div>
    <div class="field"><label for="cy-inpaint-strength">重绘强度</label><div class="strength-row"><input id="cy-inpaint-strength" v-model.number="tuning.denoise" type="range" min="0.05" max="1" step="0.05" :disabled="disabled"><output>{{tuning.denoise.toFixed(2)}}</output></div></div>
    <div class="field"><span class="field-label">重绘分辨率</span><BbiSelect v-model="tuning.resolution" :options="[{value:'auto',label:'自动（1024–1536）'},{value:'768',label:'768 × 768'},{value:'1024',label:'1024 × 1024'},{value:'1536',label:'1536 × 1536'}]" aria-label="重绘分辨率" :disabled="disabled"/></div>
    <details class="advanced"><summary>高级设置</summary>
     <div class="field"><label for="cy-inpaint-feather">边缘柔化（px）</label><input id="cy-inpaint-feather" v-model.number="tuning.feather" type="number" min="0" max="32" step="1" :disabled="disabled"></div>
     <div class="field"><span class="field-label">参考范围</span><BbiSelect :model-value="String(tuning.context)" @update:model-value="tuning.context=Number($event)" :options="[{value:'1.2',label:'较小'},{value:'1.5',label:'适中'},{value:'2',label:'较大'}]" aria-label="参考范围" :disabled="disabled"/></div>
     ...
     <div class="field"><label for="cy-inpaint-thinking">细化次数</label><input id="cy-inpaint-thinking" v-model.number="tuning.thinkingSteps" type="number" min="1" max="10" step="1" :disabled="disabled"></div>
     <div class="field"><span class="field-label">重绘优先方式</span><BbiSelect v-model="tuning.promptMode" :options="[{value:'Image First',label:'保留原图风格'},{value:'Prompt First',label:'优先满足修改要求'}]" aria-label="重绘优先方式" :disabled="disabled"/></div>
     <div class="field"><label for="cy-inpaint-seed">随机种子</label><input id="cy-inpaint-seed" v-model.number="seed" type="number" min="-1" step="1" :disabled="disabled"><p class="muted">−1 表示每次随机。</p></div>
    </details>
    <button v-if="props.saveDefaults" class="bbi-btn defaults-button" :disabled="disabled" @click="saveDefaults">保存重绘默认值</button>
   </aside>
  </div>
  <div v-else class="result-body"><div class="result-heading"><h3>重绘结果</h3><button class="bbi-btn" @click="nativeSize=!nativeSize">{{nativeSize?'适应窗口':'原尺寸对比'}}</button></div><div class="result-grid" :class="{'native-size':nativeSize}"><figure><figcaption>原图</figcaption><div class="result-image"><img :src="source" alt="重绘前的原图"/></div></figure><figure><figcaption>重绘后</figcaption><div class="result-image"><img :src="result?.url" alt="局部重绘结果"/></div></figure></div></div>
...
<style scoped>
.strength-row{display:flex;align-items:center;gap:14px}.strength-row input{flex:1;min-width:0;accent-color:var(--bbi-accent)}.strength-row output{font-variant-numeric:tabular-nums;min-width:3em;text-align:right}.parameter-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.parameter-grid .field{min-width:0}.defaults-button{width:100%;margin-top:18px;display:flex}.result-heading{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-bottom:14px}.result-heading h3{margin:0}.result-image{overflow:auto;max-height:480px}.native-size .result-image img{max-width:none;height:auto;display:block}.result-grid.native-size{align-items:start}
