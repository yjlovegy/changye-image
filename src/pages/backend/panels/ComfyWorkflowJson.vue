...
<template>
  <section class="json-editor" aria-label="工作流 JSON">
    ...
    <div class="json-status"><span class="bbi-field-hint">{{ status }} · 保存成功后自动折叠。</span><span v-if="!dirty && saved" class="json-success">✓ 已保存</span><span v-else class="json-small">{{ dirty ? '未保存' : '' }}</span></div>
    <div v-show="open" class="json-body">
      <div class="json-toolbar">
        ...
        <button type="button" class="bbi-btn" :disabled="!modelValue.trim()" title="下载已保存的工作流" @click="download">下载 JSON</button>
        <button type="button" class="bbi-btn" :disabled="configuring || dirty || !modelValue.trim()" title="先保存工作流；AI 配置会将节点和模型名发送到已配置的副 API" @click="emit('assist')">{{ configuring ? '分析中…' : 'AI 自动配置' }}</button>
        <button type="button" class="bbi-btn bbi-btn-primary json-save" :disabled="importing || stale" @click="save">保存工作流</button>
      </div>
