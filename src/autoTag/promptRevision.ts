...
export async function reviseImagePrompt(
  ...
  const spec = backendPromptSpec(settings.autoTag, mixed, characterPrompts, backend);
  ...
  const example: ImageTagContent = {
    tag: characterPrompts ? '1other, full body, three-quarter view, studio, wooden chair, easel' : 'full body, three-quarter view, ' + artistTag + ', studio',
...
${mixed ? 'tag 与 nl 必须同时非空。tag 先写主体与核心姿势、支撑和物体位置，再保留具体外貌；nl 用完整、详细且连贯的英文句子，首句先明确当前姿势、支撑及关键空间关系，再展开可见脸型五官、神态视线、外貌、穿着、动作和背景光线。只描写镜头可见的细节，保持人物归属、接触关系与姿势明确；tag/nl 必须一致。' : '当前后端采用 tag 提示词；tag 必须非空，使用英文视觉短标签。nl 保留原内容，不添加当前模型未支持的自然语言结构。'}
...
${facialDetailContract({ mixed, characterPrompts, allowDesign: true })}
...
${poseSpatialContract({ mixed, characterPrompts, negativeRequired })}
...
输出形状示例：${JSON.stringify({ images: [{ position: 'P1', ...example }] })}`;
  ...
  await requestCompletion(channel, messages, {
    ...
    validate(raw) {
      ...
      const candidate = parseImagePlan(JSON.stringify(object), [{ id: 'P1', sourceLine: 0, text: '' }], 1, 1).images[0];
      ...
      if (mixed) assertMixedPrompt({ ...revised, characters: characterPrompts ? revised.characters : [] });
